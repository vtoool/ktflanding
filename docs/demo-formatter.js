...
function describeArrivalOffset(arrivalOffset, arrivalParts) {
  if (!arrivalOffset) return '';
  const diff = parseInt(arrivalOffset, 10);
  if (!Number.isFinite(diff) || diff === 0) return '';
  const label = arrivalParts?.displayDate ? ` (${arrivalParts.displayDate})` : '';
  if (diff > 0) {
    return diff === 1 ? `Arrives next day${label}` : `Arrives ${diff} days later${label}`;
  }
  const abs = Math.abs(diff);
  return abs === 1 ? `Arrives previous day${label}` : `Arrives ${abs} days earlier${label}`;
}

function buildSegmentLine(segment, index, options) {
  ...
  return {
    ...
    pieces,
    arrivalOffset,
    arrivalNote: describeArrivalOffset(arrivalOffset, arrivalParts),
  };
}

export function formatSegmentsDetailed(segments, options = {}) {
  if (!Array.isArray(segments)) return [];
  const results = [];
  segments.forEach((segment, index) => {
    const detail = buildSegmentLine(segment, index, options);
    if (detail) {
      results.push(detail);
    }
  });
...
export function formatSegmentsToI(segments, options = {}) {
  return formatSegmentsDetailed(segments, options)
